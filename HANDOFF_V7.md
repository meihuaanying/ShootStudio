...
| 项 | 状态 |
|---|---|
| 已完成并推送 | **S0 合同** `be2c264` ｜ **S1 画面参考** `182ea8b`/`c6dd66e`/`44efb72`/`9ae31aa` ｜ **S2 显卡** `a90a202`/`b7ad285` ｜ **S3.1 three.js 升级** `2b57c7c`/`a0e836b` ｜ **S3.2 真实感** `9b2c729`/`1805425` ｜ **S3.3 布光功能** `c474cda`/`1978a91` ｜ **S3.4 相机辅助** `d235451` ｜ **S4 姿势参考图** `4157bb7` ｜ **S5-spike 识别路线** `bf9cb12` ｜ **S5 主体 端上识别集成** `1a5150c` ｜ **S6 资源库 100%** `b31ed9b` ｜ **S7 v1.3.0 发布** `397c139` |
| CI | **S6 全绿**（run 36014439005 = `b31ed9b`：Analyze ubuntu/windows + Build APK + Build Windows 4/4）；**S7 main CI 全绿**（run `36077015522` = `397c139`：Analyze ubuntu/windows + Build Windows + Build Android APK 4/4）；**release run `36077019385`**（tag `v1.3.0`）：Build & Collect (android/windows) ✅、**GitHub Release ✅**（apk 427.1MB / windows zip 303.0MB / 两个 sha256）、website 的 patch + Astro 构建 ✅、`actions/deploy-pages@v4` ❌（仓库未开启 GitHub Pages，`GET /pages` = 404，**非代码问题**，owner 开启 Pages（Source=GitHub Actions）后重跑该 job 即可）；S5 全绿（35993053989 = 1a5150c；S5-spike 35964914460 = bf9cb12）；S4 的 35953854563、S3.4 的 35855722478、S3.3 的 35839154345/35842252891 亦 success |
...
- **发布（release run `36077019385`，tag `v1.3.0`）**：commit + push → tag `v1.3.0` → `release.yml` 结果：Build & Collect (android) ✅ / Build & Collect (windows) ✅ / **GitHub Release ✅**（资产 `shoot-studio-v1.3.0-android.apk` **427.1MB**、`shoot-studio-v1.3.0-windows.zip` **303.0MB**、两个 `.sha256`）/ website 的 announcements patch + Astro 构建 + upload-pages-artifact ✅ / **Deploy website（`actions/deploy-pages@v4`）❌ = 仓库未开启 GitHub Pages**（`GET /repos/meihuaanying/ShootStudio/pages` 返回 **404**；非代码问题，**owner 需在仓库 Settings → Pages 将 Source 设为 GitHub Actions 后重跑该 job**）；main CI run `36077015522`（`397c139`）**4/4 ✅**。
...
23. **官网 Pages 部署未开启（S7 已知非代码项）**：release.yml 的 `Deploy website（GitHub Pages）` 步骤 `actions/deploy-pages@v4` 失败（run `36077019385`），根因是仓库未开启 Pages——`GET /repos/meihuaanying/ShootStudio/pages` 返回 **404**；该 job 其余步骤（announcements patch、Astro 构建、upload-pages-artifact）全部 ✅，属 owner 操作项：仓库 Settings → Pages → Source 选 **GitHub Actions**，之后重跑该 job（或对同一 tag 手动 re-run）即可发布官网。
