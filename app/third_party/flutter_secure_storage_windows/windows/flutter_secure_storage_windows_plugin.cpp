...
#include <windows.h>
#include <wincred.h>
// #include <atlstr.h>  // 已移除：VS2026 构建工具未包含 ATL；代码未使用 ATL 符号
// ---- ATL 兼容层（VS2026 Build Tools 不含 ATL；仅覆盖插件用到的 CA2W/CW2A） ----
#include <string>
namespace ss_atl_compat {
inline std::wstring toWide(const char* s) {
  if (!s) return std::wstring();
  const int len = MultiByteToWideChar(CP_UTF8, 0, s, -1, nullptr, 0);
  std::wstring out(len > 0 ? static_cast<size_t>(len - 1) : 0, L'\0');
...
inline std::string toNarrow(const wchar_t* s) {
  ...
  if (len > 0) WideCharToMultiByte(CP_UTF8, 0, s, -1, out.data(), len, nullptr, nullptr);
  return out;
}
struct CA2WX {
  std::wstring buffer;
  LPCWSTR m_psz;
  explicit CA2WX(const char* s) : buffer(toWide(s)), m_psz(buffer.c_str()) {}
};
struct CW2AX {
  std::string buffer;
  explicit CW2AX(const wchar_t* s) : buffer(toNarrow(s)) {}
  operator LPCSTR() const { return buffer.c_str(); }
};
}  // namespace ss_atl_compat
#define CA2W ss_atl_compat::CA2WX
#define CW2A ss_atl_compat::CW2AX
...
namespace
{
  ...
  NewKey:
      ...
      cred.TargetName = const_cast<LPWSTR>(target_name.m_psz);
