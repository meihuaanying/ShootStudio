...
void main() {
  ...
  group('域名门控与通道决策', () {
    ...
    test('强制直连/用户代理优先级', () async {
      ...
      expect(
        r.proxyFor(Uri.parse('https://api.pexels.com/x')),
        'PROXY 127.0.0.1:7897',
      );
      expect(
        r.proxyFor(Uri.parse('https://api.themoviedb.org/x')),
        'PROXY 127.0.0.1:7897',
      );
      await r.configure(
        userProxy: '',
  ...
  group('DoH 解析', () {
    test('JSON 解析 + TTL 缓存 + 并发去重', () async {
      int calls = 0;
      final DohResolver resolver = DohResolver(
  ...
  group('CONNECT 隧道', () {
    test('隧道盲转发：CONNECT → 200 → 字节透传', () async {
      // 上游：本地回显服务。
      final ServerSocket upstreamServer = await ServerSocket.bind(
      ...
      final DohResolver stub = DohResolver(
        httpGet: (String url) async => jsonEncode(<String, Object?>{
          'Answer': <Object?>[
            <String, Object?>{'type': 1, 'TTL': 60, 'data': '127.0.0.1'},
          ],
        }),
      );
      ...
      client.write(
        'CONNECT echo.local:$upstreamPort HTTP/1.1\r\n'
      ...
      final String text = await echoed.future.timeout(
