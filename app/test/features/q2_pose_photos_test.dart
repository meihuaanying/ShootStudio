  ...
  group('Q2 照片姿势库', () {
    ...
    test('许可与署名覆盖（D65/D71/R24）：逐图可查、禁 NC', () {
      ...
      for (final PoseEntry pose in poses) {
        ...
        expect(
          pose.license.toUpperCase(),
          isNot(contains('NC')),
          reason: '${pose.id} 禁商用许可（R24）',
        );
        expect(
          allowed,
          contains(pose.license),
    ...
    test('12 关节齐全且在限位内（R20）', () {
      for (final PoseEntry pose in poses) {
        for (final String joint in engineJoints) {
          ...
          if (base == 'elbow') {
            expect(
              v[0],
              inInclusiveRange(-150, 5),
              reason: '${pose.id} $joint rx',
            );
          } else if (base == 'knee') {
            expect(
              v[0],
              inInclusiveRange(0, 140),
              reason: '${pose.id} $joint rx',
            );
          } else if (base == 'hip') {
            expect(
              v[0],
              inInclusiveRange(-120, 40),
              reason: '${pose.id} $joint rx',
            );
      ...
      final Map<String, Object?> raw =
          (jsonDecode(
              ...
              .cast<String, Object?>();
      expect(
        (raw['clamps'] as List<Object?>?)?.isNotEmpty,
        isTrue,
    ...
    test('R20 可追溯：world3d → 12 关节（Dart 端上推导与构建期管线一致）', () {
      ...
      for (final PoseEntry pose in poses) {
        ...
        expect(
          (derived.rootPitch - pose.rootPitch).abs(),
          lessThan(1.0),
        ...
        if (calibrated is Map && calibrated['after'] is num) {
          expect(
            (pose.rootY - (calibrated['after'] as num).toDouble()).abs(),
            lessThan(0.005),
