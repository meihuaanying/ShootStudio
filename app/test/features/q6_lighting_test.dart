...
void main() {
  ...
  group('布光预设数据（D110/R50）', () {
    ...
    test('27 套预设；全部设备带 stand/offsetYaw/offsetPitch 且 rotationY=0', () {
      ...
      for (final Map<String, Object?> preset in presets) {
        ...
        for (final Map<String, Object?> device in devices) {
          expect(device['rotationY'], 0,
              reason: '${preset['id']} 仍依赖 rotationY');
          expect(device['offsetYaw'], isA<num>(),
              reason: '${preset['id']}/${device['name']} 缺 offsetYaw');
          expect(device['offsetPitch'], isA<num>(),
              reason: '${preset['id']}/${device['name']} 缺 offsetPitch');
          expect(<String>['normal', 'c', 'boom'],
          ...
          if (fixture.isNotEmpty) {
            expect(bundle.contains('"$fixture"'), isTrue,
                reason: '引擎缺少灯型 $fixture');
          }
          if (modifier.isNotEmpty) {
            expect(bundle.contains('"$modifier"'), isTrue,
    ...
    test('新增预设（美人光/发丝光/双轮廓/色光派对）存在且用新附件', () {
      final Set<String> ids =
          presets.map((Map<String, Object?> p) => '${p['id']}').toSet();
      expect(
          ids,
          containsAll(<String>[
            'clamshell',
      ...
      final Map<String, Object?> clamshell = presets
          .firstWhere((Map<String, Object?> p) => p['id'] == 'clamshell');
  ...
  group('机位模型与场景往返（D105）', () {
    test('CameraRigData 往返 + 旧场景缺 camera 字段默认值', () {
      ...
      final CameraRigData restored =
          CameraRigData.fromJson(rig.toJson().cast<String, Object?>());
    ...
    test('toEngineJson 带 camera；DeviceSpec stand/offset 往返', () {
      ...
      final Map<String, Object?> camera =
          (engine['camera'] as Map).cast<String, Object?>();
      ...
      final DeviceSpec restored =
          DeviceSpec.fromJson(spec.toJson().cast<String, Object?>());
  ...
  group('LightingController 机位/光锥（D105/D111）', () {
    ...
    test('机位变更序号递增 + POV 开关 + 场景持久化', () async {
      ...
      final Map<String, Object?> saved =
          (jsonDecode(scene.sceneJson) as Map).cast<String, Object?>();
