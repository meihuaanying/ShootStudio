...
void main() {
  ...
  group('Q1 3D 高面数与材质', () {
    ...
    test('D64：精选内置 2–4 个，其余带 downloadUrl 或 needsDownload', () {
      ...
      for (final Map<String, Object?> character in characters) {
        expect(character.containsKey('bundled'), isTrue,
        ...
        expect(hasUrl || needsDownload, isTrue,
    ...
    test('R26：轻量模式原 GLB 全部保留且非空', () {
      for (final Map<String, Object?> character in characters) {
        ...
        expect(glb.lengthSync(), greaterThan(1024),
    ...
    test('D62/D63：引擎包含细分与材质预设实现', () {
      ...
      expect(js.contains('LoopSubdivision'), isTrue,
          reason: '缺少 LoopSubdivision 特征串');
      expect(js.contains('skin-preserving-loop'), isTrue, reason: '缺少蒙皮保持细分实现');
      expect(js.contains('setSubdivision'), isTrue,
          reason: '缺少 setSubdivision API');
      expect(js.contains('setMaterialPreset'), isTrue,
      ...
      expect(bundle.lengthSync(), lessThanOrEqualTo(2200 * 1024),
    ...
    test('D61：写实模式状态可追溯，素材存在性一致', () {
      ...
      final Map<String, Object?> realistic =
          (manifest['realistic'] as Map?)?.cast<String, Object?>() ??
              <String, Object?>{};
      expect('${realistic['note'] ?? ''}'.isNotEmpty, isTrue,
      ...
      final File attribution =
          File('assets/models/characters/realistic/attribution.json');
      if (manifest['realisticAvailable'] == true) {
        ...
        final Set<String> files = attributionItems
            ...
            .toSet();
        expect(files.length, attributionItems.length,
            reason: '署名文件按 file 去重后数量不一致');
        expect(files.any((String f) => f.contains('realistic/')), isTrue,
            reason: '署名缺少写实素材条目');
      }
    });
  });
