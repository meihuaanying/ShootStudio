...
List<Object?> _goodModules() => <Object?>[
      _module('m1', 'theme', '拍摄主题', <String, Object?>{
        'text': '雨夜赛博朋克正片，霓虹与湿地反光为主基调。冷主光塑造人物轮廓，'
...
void main() {
    ...
    final RecordingAdapter adapter = RecordingAdapter(
      (RequestOptions options, String body) async {
        ...
        final List<Object?> modules =
            structuredCalls == 1 ? _poorModules() : _goodModules();
        return _json(<String, Object?>{
          'choices': <Object?>[
            <String, Object?>{
              'message': <String, Object?>{
  ...
  test('对话式修订：只改目标模块，返回 diff 供确认', () async {
            ...
            <String, Object?>{
              'message': <String, Object?>{
                'content': jsonEncode(<String, Object?>{
                  'modules': <Object?>[
                    _module('m1', 'theme', '拍摄主题', <String, Object?>{
                      'text': '改为夜景霓虹：主光 300° 冷白 2.0m，品红点缀 90°，青边光 190°；'
    ...
    // 合并结果：m1 已更新、m2 原样保留。
    final PlanModuleData mergedTheme =
        draft.modules.firstWhere((PlanModuleData m) => m.id == 'm1');
  ...
  test('本地引擎兜底：每个模块含可执行细节且质量分 ≥80（F3）', () async {
    ...
    final PlanModuleData theme = draft.modules
        .firstWhere((PlanModuleData m) => m.type == PlanModuleType.theme);
    expect((theme.data['text'] as String).length, greaterThanOrEqualTo(40));

    final PlanModuleData budget = draft.modules
        .firstWhere((PlanModuleData m) => m.type == PlanModuleType.budget);
    ...
    expect(
      budgetRows
          .whereType<Map>()
          .every((Map m) => ((m['price'] as num?)?.toDouble() ?? 0) > 0),
      isTrue,
    );
    expect('${(budgetRows.first as Map)['note']}', contains('估算值'));

    final PlanModuleData sun = draft.modules
        .firstWhere((PlanModuleData m) => m.type == PlanModuleType.sun);
    expect(sun.data['place'], isNotEmpty);
    expect(sun.data['date'], isNotEmpty);

    final PlanModuleData poses = draft.modules
        .firstWhere((PlanModuleData m) => m.type == PlanModuleType.poses);
